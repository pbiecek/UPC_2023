...
subtitle: "Day 3: Ceteris Paribus / Partial Dependence / ALE"
...
format:
  revealjs: 
    ...
    footer: BarcelonaTech Summer School -- 21/06/2023
---
...
library("rms")
load("~/GitHub/ema/docs/models/2b9b6.rda")
explain_lmr <- explain_titanic_lmr_v6

achenry$predict <- predict(explain_lmr, achenry)

library(plotly)
survival_rate <- matrix(achenry$predict, ncol = length(class))
p <- plot_ly(z = ~survival_rate) %>% add_surface()
p
...
See: [https://rml.mi2.ai/09_cp.html](https://rml.mi2.ai/09_cp.html)

![ ](figures/comic_11.png)
